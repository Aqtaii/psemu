...
static void VulkanGetExtensions(HWND window, VulkanExtensions* r) {
	EXIT_IF(window == nullptr);
	EXIT_IF(r == nullptr);

	// psemu: SDL_Vulkan_GetInstanceExtensions yerine Win32 icin gerekli iki
	// instance extension'ini dogrudan veriyoruz.
	r->required_extensions = std::vector<const char*>{
	    VK_KHR_SURFACE_EXTENSION_NAME,
	    VK_KHR_WIN32_SURFACE_EXTENSION_NAME,
	};

	std::printf("[VK-MARK] GetExtensions: vkEnumerateInstanceExtensionProperties fp=%p\n",
	            reinterpret_cast<void*>(VULKAN_HPP_DEFAULT_DISPATCHER.vkEnumerateInstanceExtensionProperties));
	std::fflush(stdout);
	...
	std::printf("[VK-MARK] GetExtensions: enumerate OK\n"); std::fflush(stdout);
...
void VulkanCreate(WindowContext* ctx) {
	...
	std::printf("[VK-MARK] VulkanCreate girisi\n"); std::fflush(stdout);
	// psemu: SDL yerine vulkan-1.dll'den vkGetInstanceProcAddr'i dogrudan yukle.
	HMODULE vk_dll = LoadLibraryW(L"vulkan-1.dll");
	if (vk_dll == nullptr) {
		EXIT("Could not load vulkan-1.dll\n");
	}
	auto get_instance_proc_addr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(
	    reinterpret_cast<void*>(GetProcAddress(vk_dll, "vkGetInstanceProcAddr")));
	if (get_instance_proc_addr == nullptr) {
		EXIT("Could not load vkGetInstanceProcAddr from vulkan-1.dll\n");
	}
	VULKAN_HPP_DEFAULT_DISPATCHER.init(get_instance_proc_addr);
	std::printf("[VK-MARK] dispatcher.init(proc_addr) OK\n"); std::fflush(stdout);

	VulkanExtensions r;
	VulkanGetExtensions(ctx->window, &r);
	std::printf("[VK-MARK] GetExtensions OK\n"); std::fflush(stdout);
	VulkanCheckInstanceVersion();
	std::printf("[VK-MARK] CheckInstanceVersion OK; createInstance...\n"); std::fflush(stdout);
	...
	std::printf("[VK-MARK] createInstance + init(instance) OK; surface...\n"); std::fflush(stdout);
	...
	// psemu: SDL_Vulkan_CreateSurface yerine Win32 surface.
	vk::SurfaceKHR::CType native_surface = VK_NULL_HANDLE;
	{
		...
		sci.hwnd      = ctx->window;
		auto fp = reinterpret_cast<PFN_vkCreateWin32SurfaceKHR>(
		    VULKAN_HPP_DEFAULT_DISPATCHER.vkGetInstanceProcAddr(
		        static_cast<vk::Instance::CType>(ctx->graphic_ctx.instance),
		        "vkCreateWin32SurfaceKHR"));
		if (fp == nullptr ||
		    fp(static_cast<vk::Instance::CType>(ctx->graphic_ctx.instance), &sci, nullptr,
		       &native_surface) != VK_SUCCESS) {
			EXIT("Could not create a Vulkan Win32 surface");
		}
	}
	...
	std::printf("[VK-MARK] createInstance+surface OK; FindPhysicalDevice...\n"); std::fflush(stdout);
	...
	std::printf("[VK-MARK] FindPhysicalDevice OK\n"); std::fflush(stdout);
	...
	std::printf("[VK-MARK] properties OK; GetSystemInfo...\n"); std::fflush(stdout);
	...
	std::printf("[VK-MARK] GetSystemInfo OK; CreateDevice...\n"); std::fflush(stdout);
	...
	std::printf("[VK-MARK] CreateDevice OK; Allocator(VMA)...\n"); std::fflush(stdout);

	if (!VulkanCreateAllocator(&ctx->graphic_ctx)) {
		EXIT("Could not create Vulkan memory allocator");
	}
	std::printf("[VK-MARK] Allocator OK; CreateQueues...\n"); std::fflush(stdout);
