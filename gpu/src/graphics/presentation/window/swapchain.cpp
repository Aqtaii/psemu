...
static void VulkanRefreshSurfaceSize() {
	...
	// psemu: SDL_Vulkan_GetDrawableSize yerine Win32 client alani.
	int  width  = 0;
	int  height = 0;
	RECT rc {};
	if (GetClientRect(g_window_ctx->window, &rc) != 0) {
...
void WindowPresentFrame(PreparedFrame* frame) {
	...
	if (g_window_ctx->window_hidden) {
		...
		ShowWindow(g_window_ctx->window, SW_SHOW); // psemu: SDL_ShowWindow -> Win32
