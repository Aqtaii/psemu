...
KYTY_SUBSYSTEM_INIT(Memory) {
	...
	VirtualMemory::Init();
	// psemu: SelfTest fatal EXIT'i uyari'ya cevrildi. Bu Kyty guest bellek
	// yoneticisinin placeholder (VirtualAlloc2) oz-testi; psemu kendi guest
	// bellegini yonettigi ve renderer guest'i direkt pointer'la okudugu icin
	// simdilik bloke olmasin. Render Kyty bellek yoneticisine muhtacsa geri
	// donulur.
	if (!g_direct_memory_backing->SelfTest()) {
		std::printf("[MEM-WARN] DirectMemoryBacking::SelfTest FAILED (non-fatal, devam)\n");
		std::fflush(stdout);
	}
